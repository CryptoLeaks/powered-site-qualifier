...
The provided Compose profile is a local starting point: loopback-only port binding, read-only root filesystem, a small non-executable temporary filesystem, dropped Linux capabilities, `no-new-privileges`, non-root UID, process limit, 512 MB memory limit, and one CPU limit. It does not mount the Docker socket, host paths, or use host networking.
