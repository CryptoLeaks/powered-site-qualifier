...
<body>
  ...
  <main id="top">
    ...
    <section id="tool" class="card"><h2>Powered Site Qualification</h2><p class="help">Use the fictional example for a fast demo, or enter your own non-confidential site information.</p><form id="qualify-form"><div class="form-grid">
      ...
      <label>Voltage / interconnection info<input id="interconnection_info" type="text" placeholder="e.g. 138 kV, interconnected"></label>
      ...
      <label style="grid-column: 1 / -1">Notes / additional information<textarea id="notes" placeholder="Notes stay in this browser and are not sent to the API."></textarea></label>
    ...
    <section id="x402" class="card" style="margin-top: 1rem"><h2>x402 Payment Demo</h2><p class="help">This browser demo sends the example payload without payment. It only demonstrates the HTTP 402 boundary and never signs or executes a payment.</p><button id="paid-button" class="secondary" type="button">Test Paid Route</button><p id="paid-status" class="status" role="status" aria-live="polite"></p><div id="payment-result" class="payment-result" hidden><div class="result-top"><h3>Status: <span class="status success">402 Payment Required</span></h3><code id="payment-error"></code></div><div class="grid" style="margin-top: 1rem"><div class="proof"><div><strong>Network:</strong> Hedera Testnet</div><div><strong>Protocol:</strong> x402 v2 exact</div><div><strong>Asset:</strong> HBAR</div></div><div class="proof"><div><strong>Amount:</strong> 0.001 HBAR / 100000 tinybars</div><div><strong>Seller:</strong> <code>0.0.10489770</code></div><div><strong>Buyer:</strong> not used for this unpaid demo</div></div></div></div></section>

    <section class="card" style="margin-top: 1rem"><h2>Successful Hedera Payment Proof</h2><p class="help">A redacted public proof from the completed test flow.</p><div class="proof"><div><strong>Transaction ID:</strong> <code>0.0.7162784@1789186391.831327025</code></div><div><strong>Buyer:</strong> <code>0.0.10488940</code></div><div><strong>Seller:</strong> <code>0.0.10489770</code></div><div><strong>Result:</strong> Blocky402 verify SUCCESS · Blocky402 settle SUCCESS · Final API HTTP 200 · Classification READY</div></div><p><a class="button secondary" href="https://github.com/CryptoLeaks/powered-site-qualifier/blob/main/evidence/hedera-real/payment-proof-redacted.json">View Successful Hedera Payment Proof</a></p></section>
